...
def get_state_dict(n, sigma_symmetry="pre"):
...
class TestV1:
    ...
    def test_direct_space_psi_builds_weight_matrix(self):
        model = nn.V1(
            ["space", "ori"],
            use_psi=True,
    ...
    def test_forward(self, f, variables, d, osi_func, osi_prob, sigma_symmetry, mode):
        model = nn.V1(
            variables,
            cell_types=(CellType.PYR, CellType.PV),
            f=f,
        ...
        if d == 1 or variables == ["space"] or mode == "matrix_approx":
    ...
    def test_forward_sample(self):
        ...
        dataset = Dataset(
            ...
            perturbations=dict(configs=[dict(N=2, cell_probs={"PYR": 1.0})]),
